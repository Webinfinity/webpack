			...
			(source, chunk, hash) => {
				...
				if (needChunkOnDemandLoadingCode(chunk)) {
					extraCode.push(
						"",
						"// .net bundler fix",
						"function getAdjustedScriptSrcPath(path) {",
						Template.indent([
							'if (window.__webpack_require_ensure_substitute_names_escape_hatch__) {',
							'return window.__webpack_require_ensure_substitute_names_escape_hatch__(path);',
							'}',
							'return path;'
						]),	
						"}",
						...
						Template.indent([
							`return ${mainTemplate.requireFn}.p + getAdjustedScriptSrcPath(${getScriptSrcPath(
								hash,
								chunk,
								"chunkId"
							)})`
